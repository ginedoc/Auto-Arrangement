	...
	for i in range(0,secNum):  # repeat leng times
		for i in range(0,len(durm_list[0])):	# one section tempo
			if(durm_list[0][i] or durm_list[1][i] or durm_list[2][i]):	
				msg_on_h = Message('note_on', note=hi_note, velocity=durm_list[0][i]*96, time=cnt*resolution, channel = 9)
				msg_on_s = Message('note_on', note=s_drum_note, velocity=durm_list[1][i]*70, time=0, channel = 9)
				msg_on_b = Message('note_on', note=b_drum_note, velocity=durm_list[2][i]*96, time=0, channel = 9)
